...
from detectron2.layers import ShapeSpec, cat, ElementWiseConv2d
...
class PB_RPNHead(nn.Module):
    ...
    def __init__(self, *, in_channels: int, num_anchors: int, box_dim: int = 4):
        ...
        super().__init__()
        # 3x3 conv for the hidden representation
        self.conv = ElementWiseConv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1)
        # 1x1 conv for predicting objectness logits
        self.objectness_logits = ElementWiseConv2d(in_channels, num_anchors, kernel_size=1, stride=1)
        # 1x1 conv for predicting box2box transform deltas
        self.anchor_deltas = ElementWiseConv2d(in_channels, num_anchors * box_dim, kernel_size=1, stride=1)
